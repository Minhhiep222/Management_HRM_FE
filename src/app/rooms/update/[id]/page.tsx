...
function UpdateDepartment() {
    ...
    const [selectedBrand, setSelectedBrand] = useState({ id: '', brand_address: '' });
    ...
    const handleGetDepartmentsByID = async (id: any) => {
        try {
            const result = await axios("http://127.0.0.1:8000/api/departments/" + id);
            setRoomField(result.data.department);
            setImg(`/images/${result.data.department.img}`)
            setSelectMember(result.data.department.members);
            setSelectedManager(result.data.department.manager);
            setSelectedBrand(result.data.department.brand);
    ...
    useEffect(() => {
        ...
        handleSelect();
        if (param) {
            handleGetDepartmentsByID(param.id);
        }
    }, []);

    const handleSetRoomField = (e: any) => {
        setRoomField({
            ...roomField,
    ...
    const handleSubmitDepartment = async (e: any) => {
        ...
        try {
            const response = await axios.put("http://127.0.0.1:8000/api/departments/" + roomField.id, roomField);
            ...
            <div className={classNames(styles["header__information-user"], styles["header__information__user-update"])}>
                <div className={styles["z_raty"]}>
                    <div className={styles["infor__user"]}>
                        <div className={styles["role__user"]} style={{ fontSize: '1.8rem' }}>Sửa Phòng Ban </div>
                    </div>
                </div>
            </div>
                                ...
                                <div className={styles["introduce"]}>
                                    ...
                                    <div className={styles["infor__project"]}>
                                        ...
                                        <div className={styles["infor__item"]}>
                                            ...
                                            <input onChange={handleSetRoomField} name="name" className={styles["input__infor"]} required placeholder="Nhập tên Phòng Ban" defaultValue={roomField.department_name} />
                                        </div>
                                        ...
                                        <div className={classNames(styles["infor__item"])}>
                                            ...
                                            <div className={styles["groups__choose"]}>
                                                <input onClick={handleSetRoomField} name="brandID" readOnly className={classNames(styles["input__infor"], styles["choosen__item-infor"])} placeholder="Chọn khu vực" defaultValue={selectedBrand.brand_address} />
                                                <div className={classNames(myspace["list__member"], styles["choose"])} >
                                        ...
                                        <div className={styles["infor__item"]}>
                                            ...
                                            <textarea onChange={handleSetRoomField} name="description" className={styles["input__infor"]} required placeholder="Nhập mô tả dự án" defaultValue={roomField.description} />
                                        </div>
                                    </div>
                                </div>
